...
currentMinute = float(dt_string.minute)
...
schedule_df = schedule_df[['Script', 'Path', 'Hour', 'Minute']]
...
for i in range(len(schedule_df)):
    ...
    #if (scriptHour == 25) & (currentMinute <= 55):
        print(scriptName)
    ...
    #elif (scriptHour == currentHour) & (currentMinute > 15) & (currentMinute < 55):
        print(scriptName)
        os.system(scriptName)
        time.sleep(15)
